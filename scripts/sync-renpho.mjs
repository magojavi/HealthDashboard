...
function toTimeString(timeStampSeconds) {
  return new Date(timeStampSeconds * 1000).toISOString().slice(11, 19);
}
...
async function main() {
  ...
  // De-dupe on (date, time): if Renpho already has that exact timestamp, skip it.
  const existingKeys = new Set(existing.map((r) => `${r[0]}|${r[14] ?? ''}`));
  const toAdd = newRows.filter((r) => !existingKeys.has(`${r[0]}|${r[14] ?? ''}`));
