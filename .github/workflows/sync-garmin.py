...
def recompute_yearly(activities):
    SPORTS = ("run", "bike", "swim", "hike")
    years = sorted(set(row[0][:4] for row in activities))
    # Zero-fill every sport for every year up front — the dashboard's
    # per-year tables expect run/bike/swim/hike to always be present, even
    # as {km:0, h:0, n:0}, and crash on a missing key otherwise.
    yearly = {year: {s: {"km": 0, "h": 0, "n": 0} for s in SPORTS} for year in years}

    for date_, type_, dist, dur, *_ in activities:
        if type_ not in SPORTS:
            continue
        year = date_[:4]
        bucket = yearly[year][type_]
